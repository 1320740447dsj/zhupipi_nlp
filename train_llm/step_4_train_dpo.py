...
if __name__ == "__main__":
    ...
    model = AutoModelForCausalLM.from_pretrained('./dsj_model/sft')

    print(f'模型可训练参数量为：{sum(p.numel() for p in model.parameters() if p.requires_grad)}')
    ref_model = AutoModelForCausalLM.from_pretrained('./dsj_model/sft').eval().to('cuda')
    ...
    args = TrainingArguments(output_dir='./dsj_model/dpo-2-epoch', 
                            ...
                            save_steps=100)          
    dataset = DPODataset('./minimind_dataset/dpo.jsonl', tokenizer=tokenizer)
    ...
    trainer.save_model('./dsj_model/dpo-2-epoch')
    trainer.save_state()